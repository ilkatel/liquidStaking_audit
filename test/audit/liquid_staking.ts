...
import { LiquidStaking, NASTR, NDistributor, NFTDistributor } from "../../typechain-types/contracts/audit";
...
describe("Algem app", function () {
    ...
    it("Print DappsStaking info", async function () {
        ...
        console.log('unbonding period:  ', unbPeriod);
    ...
    it("stake test", async function () {
        ...
        rewPerEra = eraRewards.div(2);
        expect(true).to.be.eq(aeq(signerRewardsAfter, eraRewards.div(2)));
    });
    ...
    it("transfer test", async function () {
        ...
        expect(true).to.be.eq(aeq(rewPerEra.mul(2), accRewardsAfter.sub(accRewardsBefore)));
    });
    ...
    it("harvest test", async function () {
        ...
        let accRewards = await liquidStaking.previewUserRewards(consts.util, acc.address);
    ...
    it("rewards from dapps tets", async function () {
        ...
        expect(true).to.be.eq(aeq(accRewards1, signerRewards1));
    ...
    it("withdraw test", async function () {
        for (let i = 0; i < 3; i++) {
            ...
            // await expect(liquidStaking.connect(acc).withdraw(0)).to.be.rejected;
